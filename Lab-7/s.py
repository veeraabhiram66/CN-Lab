...
#function to cost the game for 10min and display the output you time is up
def time_up():
    font = pygame.font.Font('freesansbold.ttf', 32)
    text = font.render("Time is up", True, (255, 255, 255))
...
# Function to calculate the cost for each player
#for 10min cost is 100 with 
def cost(game_state):
    for player_id, (player_x, player_y) in game_state['players'].items():
        score = game_state['player_scores'].get(player_id, 0)
        score = score + 100
        game_state['player_scores'][player_id] = score
        return game_state
